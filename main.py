...
class Listener(tweepy.Stream):
    ...
    def on_status(self, status):
        #9分を過ぎたものがでてきたらフィニッシュ
        if status.created_at.minute > aimed_minute + 8:
            sys.exit()
        try:
...
class Ranker:
    ...
    def make_img(self):
        ...
        for status in self.status_list:
            #Twitter APIのcreate_atで取れるパラメータはJSTじゃないからプラス9時間
        ...
        #もしフライングした人がいたら，フライングした人の中で一番23:29に近い人をrank_listの銭湯に加える
        if len(dq_list):
